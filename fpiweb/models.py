...
class BoxType(models.Model):
    ...
    box_type_code = models.CharField('Box Type Code', max_length=10,
        unique=True, help_text=box_type_code_help_text, )
...
class ProductCategory(models.Model):
    ...
    prod_cat_name = models.CharField('Product Category Name', max_length=30,
        unique=True, help_text=prod_cat_name_help_text, )
    ...
    prod_cat_descr = models.TextField('Product Category Description',
        null=True, help_text=prod_cat_descr_help_text, )
...
class Product(models.Model):
    ...
    prod_cat = models.ForeignKey(ProductCategory, on_delete=models.PROTECT,
        verbose_name='Product Category', help_text=prod_cat_help_text, )
...
class Box(models.Model):
    ...
    box_number = models.CharField('Visible Box Number',
        max_length=box_number_max_length, unique=True,
    ...
    box_type = models.ForeignKey(BoxType, on_delete=models.PROTECT,
        verbose_name='Type of Box', help_text=box_type_help_text, )
    ...
    loc_row = models.CharField('Row Location', max_length=2, null=True,
        blank=True, help_text=loc_row_help_text, )
    ...
    loc_bin = models.CharField('Bin Location', max_length=2, null=True,
        blank=True, help_text=loc_bin_help_text, )
    ...
    loc_tier = models.CharField('Tier Location', max_length=2, null=True,
        blank=True, help_text=loc_tier_help_text, )
    ...
    exp_year = models.IntegerField('Year Product Expires', null=True,
        blank=True, help_text=exp_year_help_text, )
    """ Year the product expires, if filled. """

    exp_month_start_help_text = (
        'Optional starting month range of when the product expires, '
        'if filled.')
    exp_month_start = models.IntegerField('Expiration Start Month (Optional)',
        null=True, blank=True, help_text=exp_month_start_help_text)
    """
    ...
    exp_month_end = models.IntegerField('Expiration End Month (Optional)',
        null=True, blank=True, help_text=exp_month_end_help_text, )
    ...
    date_filled = models.DateTimeField('Date Box Filled', null=True,
        blank=True, help_text=date_filled_help_text, )
    ...
    def get_absolute_url(self):
        return reverse('fpiweb:box_details', kwargs={'pk': self.pk}, )
...
class PalletBox(models.Model):
    ...
    PALLET_BOX_STATUS_CHOICES = (
        (NEW, 'New box added'), (ORIGINAL, 'Box already here'),
        (MOVE, 'Box being moved'),)

    id_help_text = 'Internal record identifier for a pallet box.'
    id = models.AutoField('Internal Pallet Box ID', primary_key=True,
        help_text=id_help_text, )
    ...
    product_help_text = 'Product contained in this box, if filled.'
    product = models.ForeignKey(Product, on_delete=models.PROTECT,
        verbose_name='product', help_text=product_help_text, )
    """ Product contained in this box, if filled. """

    exp_year_help_text = 'Year the product expires, if filled.'
    exp_year = models.IntegerField('Year Product Expires',
...
class Activity(models.Model):
    ...
    ADJUSTMENT_CODE_CHOICES: list = (
        ...
        (MOVE_ADDED, 'Move added box'), (CONSUME_ADDED, 'Consume added box'),)
    ...
    box_type = models.CharField('Box Type Code', max_length=10,
    ...
    date_consumed = models.DateField('Date Box Emptied', null=True, blank=True,
        help_text=date_consumed_help_text, )
    """ Date product was consumed. """

    exp_year_help_text = 'Year product would have expired.'
    exp_year = models.IntegerField('Year Expired',
        help_text=exp_year_help_text, )
    """ Year product would have expired. """

    exp_month_start_help_text = (
        'Optional starting month product would have expired.')
    exp_month_start = models.IntegerField('Start Expiration Month', null=True,
        blank=True, help_text=exp_month_start_help_text, )
    """ Optional starting month product would have expired. """

    exp_month_end_help_text = (
        'Optional ending month product would have expired.')
    exp_month_end = models.IntegerField('End Expiration Month', null=True,
        blank=True, help_text=exp_month_end_help_text, )
    """ Optional ending month product would have expired. """

    quantity_help_text = (
        'Approximate number of items in the box when it was filled.')
    quantity = models.IntegerField('Quantity in Box', default=0,
        help_text=quantity_help_text, )
    """ Approximate number of items in the box when it was filled. """
    ...
    adjustment_code = models.CharField('Adjustment Code', null=True,
        blank=True, max_length=15, choices=ADJUSTMENT_CODE_CHOICES,
...
class Constraints(models.Model):
    ...
    constraint_name = models.CharField('Constraint Name', max_length=30,
        unique=True, help_text=constraint_name_help_text, )
    ...
    constraint_type = models.CharField('Constraint Type', max_length=15,
        choices=CONSTRAINT_TYPE_CHOICES, help_text=constraint_type_help_text, )
    ...
    constraint_min = models.CharField('Minimum Valid Constraint', null=True,
        max_length=30, blank=True, help_text=constraint_min_help_text, )
    ...
    constraint_max = models.CharField('Maximum Valid Constraint', null=True,
        max_length=30, blank=True, help_text=constraint_max_help_text, )
    ...
    constraint_list = models.CharField('Valid Constraint List', null=True,
        max_length=500, blank=True, help_text=constraint_list_help_text, )
    ...
    def get_values(constraint_name):
        ...
        if constraint.constraint_type == Constraints.INT_RANGE:
            return [int(constraint.constraint_min),
                int(constraint.constraint_max), ]

        if constraint.constraint_type == Constraints.CHAR_RANGE:
            return [constraint.constraint_min, constraint.constraint_max, ]
...
class ProductExample(models.Model):
    ...
    prod_example_name = models.CharField('Product Example Name', max_length=30,
